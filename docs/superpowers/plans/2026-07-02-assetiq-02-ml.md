...
def build_preprocessor() -> ColumnTransformer:
    return ColumnTransformer(
        transformers=[
            # drop="first" removes the dummy-variable trap so the LinearRegression
            # baseline stays numerically stable; still compatible with
            # handle_unknown="ignore" (unknown categories encode as all-zeros).
...
Expected: all pass (features + ingest + metrics + train + predict). Note: review hardening added three regression tests (features, ingest, train), so the delivered count is 35 (13 + 9 + 6 + 4 + 3), not the 32 of the plan's original per-task blocks.
